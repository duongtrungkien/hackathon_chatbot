...
class FastTextFeaturizer(DenseFeaturizer):
    ...
    def __init__(self, component_config: Optional[Dict[Text, Any]] = None) -> None:
        ...
        if "cache_dir" not in component_config.keys():
            raise ValueError(
                "You need to set `cache_dir` for the FasttextFeaturizer.")
        if "file" not in component_config.keys():
            raise ValueError(
                "You need to set `file` for the FasttextFeaturizer.")
        path = os.path.join(
            component_config["cache_dir"], component_config["file"])
    ...
    def set_fasttext_features(self, message: Message, attribute: Text = TEXT) -> None:
        ...
        text_vector = self.model.get_word_vector(
            message.get(TEXT)).reshape(1, -1)
        word_vectors = np.array(
            [self.model.get_word_vector(t.text) for t in tokens])
